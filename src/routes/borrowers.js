...
router.delete('/:id', async (req, res) => {
  try {
    ...
    // Check if borrower has active (unreturned) loans
    const [activeLoans] = await pool.query(
      "SELECT COUNT(*) as count FROM transactions WHERE borrower_id = ? AND LOWER(status) IN ('loaned', 'overdue')",
      [req.params.id]
    );

    if (activeLoans[0].count > 0) {
      return res.status(400).json({
        error: `Cannot delete: borrower has ${activeLoans[0].count} active loan(s). Please return all books first.`
      });
    }

    // Delete all related records that reference this borrower
    await pool.query('DELETE FROM transactions WHERE borrower_id = ?', [req.params.id]);
    await pool.query('DELETE FROM reservations WHERE borrower_id = ?', [req.params.id]);
